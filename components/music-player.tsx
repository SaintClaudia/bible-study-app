...
// ── Spotify iframe — always clips corners correctly on WebKit ───
...
// ── Collapsed bar (rendered inside the nav in AppShell) ─────────
// Lives inside the nav so it auto-hides with it on scroll.
  ...
  return (
    <div className="border-b border-border">
      <div className="mx-auto max-w-2xl flex items-center gap-3 px-4 py-3">
        ...
        <button type="button" onClick={() => setPlayerExpanded(true)} className="p-2 text-muted-foreground hover:text-foreground active:opacity-60" aria-label="Open player">
          <ChevronUp className="h-5 w-5" />
        </button>
        <button type="button" onClick={() => setNowPlaying(null)} className="p-2 text-muted-foreground hover:text-foreground active:opacity-60" aria-label="Close player">
          <X className="h-4 w-4" />
        </button>
      </div>
    </div>
...
function SDKExpandedPlayer() {
  const {
    nowPlaying, setNowPlaying, setPlayerExpanded,
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col bg-background">
      ...
      <div className="mx-auto w-full max-w-2xl flex items-center justify-between px-5 py-4 border-b border-border flex-shrink-0">
        <button type="button" onClick={() => setPlayerExpanded(false)} className="p-2 -ml-2 text-muted-foreground hover:text-foreground active:opacity-60">
          <ChevronDown className="h-5 w-5" />
        </button>
        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Now Playing</p>
        <button type="button" onClick={() => setNowPlaying(null)} className="p-2 -mr-2 text-muted-foreground hover:text-foreground active:opacity-60">
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-10 gap-8">
        ...
        <div className="flex items-center gap-8">
          ...
          <button type="button" onClick={togglePlay} className="h-16 w-16 rounded-full bg-foreground flex items-center justify-center text-background hover:opacity-80 active:opacity-60">
            {isPaused
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col bg-background">
      ...
      <div className="mx-auto w-full max-w-2xl flex items-center gap-3 px-4 py-4 border-b border-border flex-shrink-0">
        ...
        <button type="button" onClick={() => setPlayerExpanded(false)} className="p-2 -mr-1 text-muted-foreground hover:text-foreground active:opacity-60" aria-label="Minimize">
          <ChevronDown className="h-5 w-5" />
        </button>
        <button type="button" onClick={() => setNowPlaying(null)} className="p-2 text-muted-foreground hover:text-foreground active:opacity-60" aria-label="Close">
          <X className="h-4 w-4" />
        </button>
      </div>
...
// ── Main export — only the expanded overlay ─────────────────────
// The collapsed bar lives in AppShell's nav via <MiniPlayerBar />.
