...
import { fetchSaintOfTheDay, fetchNextSaint, type SaintOfTheDay } from '@/lib/saints'
...
export function SaintTab() {
  ...
  const [nextSaint, setNextSaint] = useState<{ date: Date; saint: SaintOfTheDay } | null>(null)
  ...
  useEffect(() => {
    ...
    setDisplayDate(date)
    fetchSaintOfTheDay(date).then(async (s) => {
      setSaint(s)
      if (!s) setNextSaint(await fetchNextSaint(date))
      setLoading(false)
    })
  }, [])
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {saint && <SaintContent saint={saint} />}
